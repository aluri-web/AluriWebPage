...
import { Store, MapPin, Search, SlidersHorizontal, X, ArrowUpDown } from 'lucide-react'
...
const calculateRiskScore = (loan: DemoCredito) => {
...
export default function DemoMarketplacePage() {
  // Filter creditos to only published ones
  const allLoans = useMemo(
    () => DEMO_CREDITOS.filter((c) => c.estado === 'publicado'),
    []
  )

  const [filteredLoans, setFilteredLoans] = useState<DemoCredito[]>(allLoans)
  ...
  }, [searchTerm, riskFilter, sortOption, allLoans])
  ...
  const getPropertyTitle = (loan: DemoCredito) => {
    const propertyType = loan.tipo_inmueble || 'Inmueble'
    const city = loan.ciudad_inmueble || 'Colombia'
    return `${propertyType} ${city}`
  }

  const getFundingProgress = (loan: DemoCredito) => {
    const requested = loan.monto_solicitado || 0
    const funded = DEMO_INVERSIONES
      .filter(
        (i) =>
          i.credito_id === loan.id &&
          (i.estado === 'activo' || i.estado === 'pendiente')
      )
        ...
        {filteredLoans.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredLoans.map((loan) => {
              ...
              const imageUrl =
                loan.fotos_inmueble && loan.fotos_inmueble.length > 0
                ...
                >
                  ...
                  <div className="relative h-44 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span
                        className={`px-3 py-1.5 ${risk.bgColor} ${risk.textColor} border ${risk.borderColor} text-xs font-semibold rounded-full backdrop-blur-sm`}
                      >
                        RIESGO {risk.score}
                      </span>
                    </div>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex gap-3">
                      <Link
                        href={`/dashboard/demo/inversionista/marketplace/${loan.id}`}
          ...
          <div className="flex flex-col items-center justify-center h-80 bg-[#111] rounded-2xl border border-white/5">
            ...
            <p className="text-gray-600 text-sm mt-2">
              Vuelve pronto para ver nuevas inversiones
            </p>
          </div>
        )}
      </div>
