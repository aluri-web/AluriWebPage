...
import { DEMO_CREDITOS, DEMO_INVERSIONES, DEMO_TRANSACCIONES, formatCOP } from '@/lib/demo-data/index'
...
export default function DemoCreditosPage() {
  ...
  const getTransacciones = (creditoId: string) => {
    return DEMO_TRANSACCIONES.filter(t => t.credito_id === creditoId)
  }
              ...
              <div key={credito.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <button
                        onClick={() => alert('Funcion de pago no disponible en modo demo')}
                  ...
                  {/* Inline Abonos Table */}
                  {transacciones.length > 0 && expandedAbonos.has(credito.id) && (
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-gray-50 text-left">
                            ...
                            <th className="px-4 py-3 text-gray-500 font-medium">Referencia</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                          {transacciones.map(tx => (
                            <tr key={tx.id} className="hover:bg-gray-50">
                              <td className="px-4 py-3 text-gray-900">{formatDate(tx.fecha_aplicacion)}</td>
                              <td className="px-4 py-3">
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                                  ...
                                  {tx.tipo_transaccion === 'pago_capital' ? 'Capital' : tx.tipo_transaccion === 'pago_interes' ? 'Interes' : tx.tipo_transaccion}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-right font-medium text-gray-900">{formatCOP(tx.monto)}</td>
                              <td className="px-4 py-3 text-gray-500">{tx.referencia_pago || '-'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
