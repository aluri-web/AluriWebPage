...
export interface ExtractoInversion {
  codigo_credito: string
  monto_invertido: number
  porcentaje: number
  pagos: ExtractoPago[]
  subtotal: { capital: number; interes: number; mora: number; total: number }
}
...
export async function getExtractoMensual(
  mes: number,
  anio: number
  ...
  // Fetch active/liquidated investments with credit info
  const { data: inversiones, error: invError } = await supabase
    ...
    .in('estado', ['activo', 'liquidado'])

  if (invError) {
    return { data: null, error: 'Error al obtener inversiones' }
  }
  ...
  const extractoInversiones: ExtractoInversion[] = []

  for (const inv of inversiones) {
    const creditoData = inv.credito as unknown as {
      codigo_credito: string
    ...
    const porcentaje = creditoData.monto_solicitado > 0
      ? (inv.monto_invertido / creditoData.monto_solicitado) * 100
      : 0
    ...
    if (!transacciones || transacciones.length === 0) continue
  ...
  const totales = extractoInversiones.reduce(
    (acc, inv) => ({
      capital: acc.capital + inv.subtotal.capital,
      interes: acc.interes + inv.subtotal.interes,
      mora: acc.mora + inv.subtotal.mora,
      total: acc.total + inv.subtotal.total,
    }),
    { capital: 0, interes: 0, mora: 0, total: 0 }
  )
  ...
  return {
    data: {
      ...
      periodo: { mes, anio, label: `${MESES[mes - 1]} ${anio}` },
      inversiones: extractoInversiones,
      totales,
    },
    error: null
  }
