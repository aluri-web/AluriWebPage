...
export async function impersonateUser(userId: string, userRole: string): Promise<{ url?: string; error?: string }> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'

  if (!supabaseUrl || !serviceRoleKey) {
    return { error: 'Configuracion del servidor incompleta.' }
  }

  const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey)

  // Determine redirect based on role
  let redirectTo = '/dashboard/inversionista'
  if (userRole === 'propietario') {
    redirectTo = '/dashboard/propietario'
  ...
  if (linkError) {
    ...
    return { error: 'Error al generar enlace de acceso.' }
  }
